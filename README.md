...
options:
  ...
  --target TARGET       Option to only reduce a specific target. String used here must be contained within the target name in file headers. Optional parameter.
  ...
  --fieldcenter FIELDCENTER FIELDCENTER
                        Align the output images to this center coordinate. This is useful for difference imaging where the frames need to be a common size, pixel scale, and set of
                        coordinates.
...
*_centroids.png  red/workspace  A data quality image used to calculate the dispersion in WCS solution (see: `potpyri/solve_wcs.py`)
...
*stk.epsf.png    red            A data quality image from the effective PSF generated by `potpyri/photometry.py`
```

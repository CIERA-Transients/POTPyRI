...
def quality_check(aligned_images, aligned_data, tel,
    nsource_align=100, nsource_align_min=20, log=None):

    t_start = time.time()

    if log: log.info('Running quality_check version '+str(__version__))
    ...
    for f in aligned_images:
        if log: log.info('Loading catalog for: '+f)
        ...
        if len(table)==0:
            if log: log.info('No sources found by SExtractor.')
            fwhm.append(0)
            elong.append(0)
            stars.append(0)
            ...
            stars.append(match)
            if log: log.info('FWHM of image in arcsec: '+str(fwhm_image))
            if log: log.info('Elongation of image: '+str(elong_image))
    
    if log: log.info('Calculating median parameters.')
    fwhm_med = np.median(fwhm)
    fwhm_std = np.std(fwhm)
    if log: log.info('Median FWHM of images in arcsec = %.3f +/- %.3f'%(fwhm_med,fwhm_std))
    elong_med = np.median(elong)
    elong_std = np.std(elong)
    if log: log.info('Median Elongation of images = %.3f +/- %.3f'%(elong_med,elong_std))

    if log: log.info('Using 3 sigma cuts to calculate the number of bad images.')
    fwhm = np.array(fwhm)
    ...
    if log: log.info('Number of images with FWHM > 3sigma = %d'%n_fwhm)
    if log: log.info('Number of images with elongation > 3sigma = %d'%n_elong)

    ten_percent = int(len(aligned_images)*0.1)
    if n_fwhm > ten_percent or n_elong > ten_percent:
        if log: log.info('More than 10 percent of the images have bad quality.')
        if log: log.info('Restricting cuts to 10 percent.')
    ...
    if log: log.info('Calculating mid-time and total exposure time of stack.')
    time_list = []
    total_time = 0

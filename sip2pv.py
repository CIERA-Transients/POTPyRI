...
if __name__ == '__main__':
    ...
    parser.add_option('--input', default=None, type="string", help='path to input file with SIP distortion')
    parser.add_option('--output', default=None, type="string", help='path to output file')

    usagestring='USAGE: sip2pv.py'
    options, args = parser.parse_args()
    infile = options.input
    outfile = options.output

    hdu = fits.open(infile)
    header = copy(hdu[0].header)
    sip_to_pv(header)
    hdu[0].header = header
    hdu.writeto(outfile)
